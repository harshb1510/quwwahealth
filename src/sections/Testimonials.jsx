...
import imageJohn from '../assets/images/Testimonials/image.png';
import imageEllen from '../assets/images/Testimonials/image1.png';
...
const Testimonials = () => {
  return (
    <section className="py-12 md:py-20 bg-white border-t-4 border-t-green-50">
          ...
          >
            {testimonials.map((testimonial, index) => (
              <SwiperSlide key={index}>
                  {({ isActive }) => (
                      <div className={`transition-transform duration-500 ease-in-out flex flex-col items-center relative ${isActive ? 'scale-100' : 'scale-75'}`}>
                          <img src={testimonial.image} alt={testimonial.name} className="max-w-[280px] h-auto object-contain" />
                          <div className={`absolute -bottom-8 w-56 h-20 bg-[#F3F25B] flex items-center justify-center transition-opacity duration-500 ${isActive ? 'opacity-100' : 'opacity-0 md:opacity-100'}`}
                               style={{ clipPath: 'polygon(0% 0%, 100% 0%, 85% 100%, 15% 100%)' }}>
                              <p className="font-bold text-xl text-black">{testimonial.name}</p>
                          </div>
                      </div>
                  )}
              </SwiperSlide>
